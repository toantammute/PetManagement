...
import AsyncStorage from "@react-native-async-storage/async-storage";
    ...
    export const deletePet = async (id: string): Promise<Pet> => {
        ...
        try {
            ...
            console.log('response.data of deletePet', response.data);
            console.log('response.status of deletePet', response.status);
            return response.data;
        } catch (error: any) {
    ...
    export const updatePet = async (pet: Pet, image: Image, id: string): Promise<Pet> => {
        ...
        try {
            const response = await axios.put(`${API}/pet/update/${id}`, formData, {
                headers: {
                    'Content-Type': 'multipart/form-data',
                    'Accept': 'application/json',
                    'Authorization': `Bearer ${accessToken}`
